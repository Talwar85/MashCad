...
# Gate-Evidence Generator - W13 Unskip + Retest Edition
...
# Default output prefix if not specified (W13)
if (-not $OutPrefix) {
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $OutPrefix = "roadmap_ctp/QA_EVIDENCE_W13_$timestamp"
}

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$rootDir = Split-Path -Parent $scriptDir

Write-Host "=== Gate-Evidence Generator (W13) ===" -ForegroundColor Cyan
...
$jsonData = @{
    metadata = @{
        ...
        evidence_version = "3.1"  # W13: Same schema, updated numbers (drag tests runnable)
        toolchain = @{
            python = "3.11.14"
            pytest = "9.0.2"
            platform = "win32"
    ...
    signatures = @{
        sha256_equivalent = "$($coreParsed.passed)p$($coreParsed.skipped)s_$($pi10Parsed.passed)p_$($uiParsed.errors)e$($hygieneViolations)v_w13_$(Get-Date -Format 'yyyyMMdd')"
    }
}
...
**Generated by:** AI-3 (GLM 4.7) QA Cell - **Automated W13**
...
Write-Host ""
Write-Host "=== Evidence Files Generated (W13) ===" -ForegroundColor Green
