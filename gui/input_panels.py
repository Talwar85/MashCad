...
class PushPullInputPanel(QFrame):
    """Input panel for interactive PushPull — face extrude along normal."""
    ...
    def __init__(self, parent=None):
        ...
        self.dist_input = ActionSpinBox()
        self.dist_input.setRange(-500.0, 500.0)
        self.dist_input.setDecimals(2)
        self.dist_input.setSuffix(" mm")
        self.dist_input.setValue(10.0)
        self.dist_input.setSingleStep(1.0)
        self.dist_input.valueChanged.connect(self._on_dist_changed)
        self.dist_input.enterPressed.connect(self._confirm)
        self.dist_input.escapePressed.connect(self.cancelled.emit)
        layout.addWidget(self.dist_input)

        self.btn_ok = QPushButton("OK")
        self.btn_ok.setStyleSheet("background: #50bb50; color: #000; border: none; font-weight: bold;")
        self.btn_ok.clicked.connect(self._confirm)
        layout.addWidget(self.btn_ok)
    ...
    def _confirm(self):
        self._distance = self.dist_input.value()
        self.confirmed.emit()

    def get_distance(self) -> float:
        return self.dist_input.value()
