...
class SketchHandlersMixin:
    ...
    def _handle_trim_legacy(self, pos, snap_type, snap_entity=None):
        """
        DEPRECATED: Alte Trim-Implementierung, nur fÃ¼r Fallback.
        """
        # --- Imports Setup ---
        try:
            import sketcher.geometry as geometry
        ...
        # 2. Liste aller anderen Entities erstellen (FIX FÃœR AttributeError)
        # Wir mÃ¼ssen die Listen manuell zusammenfÃ¼gen, da self.sketch.entities nicht existiert
        other_entities = []
        ...
        # Loop Ã¼ber die manuell erstellte Liste
        ...
        if isinstance(target, Line2D):
            ...
            if not cut_points:
                ...
                for i in range(n):
                    ...
                    # PrÃ¼fe ob Maus im Segment liegt
                    if t_s < t_e:
                        ...
                        # Wrap-Around Segment (geht Ã¼ber 0)
        ...
        if segment_to_remove:
            ...
            if QApplication.mouseButtons() & Qt.LeftButton:
                ...
                # WICHTIG: Constraints fÃ¼r diese Entity entfernen BEVOR die Entity gelÃ¶scht wird!
                if hasattr(self.sketch, 'remove_constraints_for_entity'):
                    removed_count = self.sketch.remove_constraints_for_entity(target)
                    if removed_count > 0:
                        logger.debug(f"Trim: {removed_count} Constraints entfernt fÃ¼r {type(target).__name__}")
                ...
                # Neu erstellen (Was Ã¼brig bleibt)
                if isinstance(target, Line2D):
                    for i in range(len(cut_points) - 1):
                        p_start = cut_points[i][1]
                        p_end = cut_points[i+1][1]
                        
                        # Check ob das das gelÃ¶schte Segment ist
                    ...
                    # get_param_on_entity gibt Radiant zurÃ¼ck
