...
from config.feature_flags import get_setting, is_enabled
...
class SketchEditor(QWidget, SketchHandlersMixin, SketchRendererMixin):
    ...
    def __init__(self, parent=None):
        ...
        # SU-005: Use runtime setting for throttle interval (default 16ms for 60 FPS)
        self._direct_edit_live_solve_interval_s = get_setting("sketch_solver_throttle_ms", 16) / 1000.0
        ...
        # W35: Home-Button fÃ¼r schnellen ZurÃ¼cksprung zum Origin
        self._btn_home = QToolButton(self)
        self._btn_home.setText("ðŸ ")
    ...
    def _update_slot_constraints_after_drag(self, slot_line, mode):
        ...
        KORREKTE STRATEGIE:
        1. Bei Center/Length: Constraints bleiben gleich, nur Positionen Ã¤ndern sich
        ...
        # Nur bei Radius-Change mÃ¼ssen wir Constraints aktualisieren
        if mode != "radius":
            return
        ...
        for c in self.sketch.constraints:
            if c.type == ConstraintType.DISTANCE:
                entities = c.entities
                if len(entities) >= 2:
                    # PrÃ¼fe ob dies ein Slot-Radius-Constraint ist
    ...
    def snap_point(self, w):
        ...
        if self.snapper:
            ...
            # W35: Snappoints fÃ¼r bereits geklickte Spline-Punkte wÃ¤hrend der Erstellung
            if self.current_tool == SketchTool.SPLINE and self.tool_points:
                ...
                # Wenn tool_point nÃ¤her ist als der gefundene Snappoint, nutze tool_point
                if closest_tool_point and min_dist < snap_radius_world:
                    # PrÃ¼fe auch die Distanz zum Snapper-Ergebnis
                    ...
                    if min_dist < snapper_dist:
                        ...
                        self.last_snap_priority = 100  # Hohe PrioritÃ¤t
    ...
    def _resolve_direct_edit_target_slot(self):
        ...
        hit_tolerance = max(20.0 / self.view_scale, 10.0)  # Adaptive tolerance (grÃ¶ÃŸer fÃ¼r bessere Treffer)
    ...
    def _resolve_direct_edit_target_spline(self):
        ...
        # PrÃ¼fe ob gehovered Entity ein Spline ist
    ...
    def _get_spline_center_point(self, spline):
        """
        Berechnet den visuellen Mittelpunkt eines Splines fÃ¼r das Direct Edit Handle.
        """
    ...
    def _start_direct_edit_drag(self, handle_hit):
        ...
        # Da die Direct-Edit-Infrastruktur spline_dragging nicht zurÃ¼cksetzt,
        # wÃ¼rde der User sonst "gefangen" sein (Drag kann nicht beendet werden)
        if kind == "spline":
            self._save_undo()
    ...
    def _apply_direct_edit_drag(self, world_pos, axis_lock=False):
        ...
        if self._direct_edit_slot is not None:
            ...
            if mode == "center":
                ...
                # WICHTIG: Arc-Center auch verschieben (fÃ¼r Live-Visualisierung)
                # Die Coincident-Constraints synchronisieren sie spÃ¤ter
                for arc in self.sketch.arcs:
                    if hasattr(arc, '_slot_arc') and arc._slot_arc:
                        arc.center.x += dx
                        arc.center.y += dy
                ...
                if mode == "length_start":
                    ...
                    slot.start.x += dx
                    slot.start.y += dy
                    # WICHTIG: ZugehÃ¶rigen Arc-Center auch verschieben
                    for arc in self.sketch.arcs:
                        if hasattr(arc, '_slot_arc') and arc._slot_arc:
                            dist_to_start = math.hypot(arc.center.x - slot.start.x, arc.center.y - slot.start.y)
                ...
                if arc is not None:
                    ...
                    # WICHTIG: Start-Position aktualisieren fÃ¼r kontinuierliches Dragging
    ...
    def _finish_direct_edit_drag(self):
        ...
        W35 P4: Endet Incremental Solver und fÃ¼hrt Final-Solve durch.
        """
        if not self._direct_edit_dragging:
            return
        ...
        if moved:
            ...
            # WICHTIG: Arc-Winkel aktualisieren (fÃ¼r Slots und andere Arc-Features)
    ...
    def _finish_spline_drag(self):
        ...
        spline._lines = new_lines  # Nur Cache fÃ¼r Rendering/Hittest - nicht zu sketch.lines!

        # WICHTIG: Segmente NICHT zu sketch.lines hinzufÃ¼gen!
        # Spline als Ganzes ist bereits in sketch.splines - Segmente sind nur fÃ¼r Rendering.
        ...
        if self.is_panning:
            ...
            if self.current_tool == SketchTool.SELECT:
                ...
                if spline_elem != self.hovered_spline_element:
                     ...
                     if spline_elem:
                         # NOTE: BBox der ganzen Spline kÃ¶nnte hier geholt werden fÃ¼r
                         # optimierten Partial-Update. Aktuell wird aus Einfachheit
                         # needs_full_update=True verwendet, da BBox-Berechnung
                         # Ã¤hnlich teuer wie Full-Update bei typischen Spline-GrÃ¶ÃŸen.
                         pass
    ...
    def _drag_spline_element(self, shift_pressed):
        ...
        if self.spline_drag_type == 'body':
             ...
             # WICHTIG: Auch _preview_lines setzen fÃ¼r Live-Rendering!
    ...
    def _finish_spline(self):
        """
        Finalisiert die Spline-Erstellung und fÃ¼gt den Spline zum Sketch hinzu.
        ...
        spline.closed = False  # TODO: Geschlossene Splines kÃ¶nnten spÃ¤ter Option sein

        # Alle Punkte aus tool_points hinzufÃ¼gen
        for pt in self.tool_points:
        ...
        # Preview-Lines fÃ¼r Rendering setzen
        spline._lines = spline.to_lines(segments_per_span=10)

        # Tool-Status zurÃ¼cksetzen
        self.tool_step = 0
        self._last_auto_show_step = -1
        self.tool_points.clear()
        self.tool_data.clear()

        # ZurÃ¼ck zum Select-Tool wechseln
        self.set_tool(SketchTool.SELECT)

        # Spline selektieren fÃ¼r sofortiges Editieren
        self.selected_splines = [spline]
    ...
    def _delete_selected(self):
        ...
        # W35-fix: Cascade delete fÃ¼r Slots
        ...
        # ZusammenfÃ¼hren der zu lÃ¶schenden Elemente
        ...
        # Eigentliches LÃ¶schen durchfÃ¼hren
    ...
    def _is_spline_segment_line(self, entity) -> bool:
        ...
        # Fallback fÃ¼r Ã¤ltere ZustÃ¤nde ohne Marker
    ...
    def _entity_passes_selection_filter(self, entity) -> bool:
        ...
        # WICHTIG: Center-Line (_slot_center_line) bleibt selektierbar - sie reprÃ¤sentiert den Slot!
    ...
    def _entity_pick_priority(self, entity) -> int:
        # Ellipse-Handle-Punkte haben hÃ¶chste PrioritÃ¤t (niedrigste Nummer)
        if isinstance(entity, Point2D) and getattr(entity, '_ellipse_handle', None) is not None:
            return -1  # HÃ¶chste PrioritÃ¤t fÃ¼r Ellipse-Handles
        ...
        # W35: Slot-Komponenten haben niedrigste PrioritÃ¤t (werden fast nie selektiert)
        # WICHTIG: Center-Line hat NORMALE PrioritÃ¤t (0), damit sie selektierbar ist!
        if isinstance(entity, Line2D) and getattr(entity, '_slot_skeleton_line', False):
    ...
    def _delete_spline_point(self, spline, idx):
        ...
        # WICHTIG: Cache invalidieren nach Ã„nderung der control_points!
        spline.invalidate_cache()
        # Re-calc lines
        spline._lines = spline.to_lines(segments_per_span=10)
        self.sketched_changed.emit()
    ...
    def _toggle_spline_closed(self, spline, closed):
        ...
        # WICHTIG: Cache invalidieren nach Ã„nderung!
        spline.invalidate_cache()
        # Re-calc lines
        spline._lines = spline.to_lines(segments_per_span=10)
        self.sketched_changed.emit()
    ...
    def _insert_spline_point(self, spline, pos):
        ...
        if best_idx != -1:
            ...
            # WICHTIG: NICHT zu sketch.lines hinzufÃ¼gen!
    ...
    def _draw_direct_edit_handles(self, painter):
        ...
        # PrÃ¼fe erst, ob ein Slot selektiert ist (fÃ¼r persistente Handle-Anzeige)
        slot = None
        slot_source = None
        ...
        # Wenn kein Slot selektiert, prÃ¼fe Hover
        if slot is None:
            slot, slot_source = self._resolve_direct_edit_target_slot()
        ...
        # W35: Spline Handles (Center Handle fÃ¼r direktes Verschieben)
