...
class CadQueryImporter:
    ...
    def create_bodies_from_solids(self, solids: List[Any], name: str = "CadQuery Import", script_source: str = "", script: str = "") -> List:
        """
        Create Body objects from Build123d solids with CadQueryFeature.

        Args:
            solids: List of Build123d Solid objects
            name: Base name for the bodies
            script_source: Source script filename for reference
            script: The actual script content (for editable feature)

        Returns:
            List of Body objects with CadQueryFeature attached
        """
        from modeling import Body
        from modeling.features import CadQueryFeature
        ...
        for i, solid in enumerate(solids):
            ...
            # Create CadQueryFeature (editable, stores script)
            feature = CadQueryFeature(
                name=f"Script {script_source or name}",
                script=script,
