...
try:
    from .visual_mesh_analyzer import VisualMeshAnalyzer
    HAS_VISUAL_ANALYZER = True
except ImportError:
    HAS_VISUAL_ANALYZER = False
    logger.warning("Visual Mesh Analyzer not available")
...
class STLFeatureAnalyzer:
    ...
    NOW WITH VISUAL PROJECTION DETECTION - No more guessing!
    """
    ...
    def __init__(self, quality_checker: Optional[MeshQualityChecker] = None):
        ...
        if HAS_VISUAL_ANALYZER:
            logger.info("STLFeatureAnalyzer initialized with Visual Mesh Analyzer")
    ...
    def _detect_base_plane(self, mesh: 'pyvista.PolyData') -> Optional[PlaneInfo]:
        ...
        NEW: Visual Projection Detection (highest priority) - No more guessing!
        Prioritizes Visual > Point Cloud (Alpha Shape) > Slicing > RANSAC > Legacy.
        """
        # 0. Option: Slice (Z-Min Footprint) - HIGHEST PRIORITY for parts with holes
        slice_plane = self._detect_base_plane_slice(mesh)
        if slice_plane:
            logger.info(f"Slice-Plane found: {slice_plane.area:.0f}mm² (most robust for parts with holes)")
            return slice_plane

        # 1. Option: Visual Projection (good for solid parts)
        if HAS_VISUAL_ANALYZER:
            try:
                visual_plane = self._detect_base_plane_visual(mesh)
                if visual_plane:
                    logger.info(f"Visual Plane found: {visual_plane.area:.0f}mm² (method={visual_plane.detection_method})")
                    return visual_plane
            except Exception as e:
                logger.warning(f"Visual plane detection failed: {e}")

        # 2. Option: Point Cloud Alpha Shape (User Request: "Connect the dots at bottom")
    ...
    def _detect_base_plane_visual(self, mesh: 'pyvista.PolyData') -> Optional[PlaneInfo]:
        """
        Detect base plane using visual projection analysis (NEW - No more guessing!).

        Strategy:
        1. Create 2D projections (Top, Side, Front)
        2. Apply visual algorithms (ConvexHull, MinAreaRect)
        3. Find dominant planar face by analyzing projection area ratios
        4. Extract exact boundary using contour extraction
        """
        try:
            # Use VisualMeshAnalyzer
            visual_analyzer = VisualMeshAnalyzer(mesh)
            result = visual_analyzer.analyze_mesh(mesh)

            # Find dominant projection (largest area = most likely base plane)
            projections = result.get("projections", [])
            if not projections:
                return None

            # For mechanical parts, the base is usually the smallest dimension (thickness)
            # So we look for the projection with largest AREA (footprint)
            dominant_proj = max(projections, key=lambda p: p.area_2d)

            logger.info(f"Visual detection: dominant projection={dominant_proj.name}, area={dominant_proj.area_2d:.0f}mm²")

            # Extract contours from dominant projection
            contours = visual_analyzer._extract_contours_from_projection(dominant_proj)

            if not contours:
                # Fallback: MinAreaRect
                rect = visual_analyzer._detect_min_area_rect(dominant_proj)
                if rect:
                ...
                boundary_points = visual_analyzer.backproject_contour_to_3d(outer_contour, dominant_proj)
                area = outer_contour.area
                confidence = min(0.95, outer_contour.convexity + 0.1)
            ...
            if boundary_points:
                ...
                origin = dominant_proj.plane_origin

            return PlaneInfo(
                origin=origin,
                normal=dominant_proj.plane_normal,
                area=float(area),
                boundary_points=[tuple(p) for p in boundary_points],
                confidence=float(confidence),
                detection_method=f"visual_projection_{dominant_proj.name}",
                enabled=True
            )

        except Exception as e:
            logger.warning(f"Visual base plane detection failed: {e}")
            import traceback
    ...
    def _detect_base_plane_points(self, mesh: 'pyvista.PolyData') -> Optional[PlaneInfo]:
        ...
        try:
            ...
            # Actually, for a footprint, we usually want Convex Hull UNLESS there are large cutouts.
            ...
            if not valid_triangles:
                # Fallback to Convex Hull if no dense triangles found
                hull = mp.convex_hull
                ...
                # Cleanup: Close holes?
                # For base plane, we usually want OUTER boundary.
    ...
    def _detect_plane_ransac(self, mesh: 'pyvista.PolyData') -> Optional[PlaneInfo]:
        ...
        try:
            ...
            # Wir nutzen Face-Centers als Punktewolke für RANSAC
    ...
    def _detect_base_plane_legacy(self, mesh: 'pyvista.PolyData') -> Optional[PlaneInfo]:
        ...
        try:
            ...
            for normal, origin, name in candidates:
                ...
                # Calculate vector from face center to plane origin
                # dist = dot(center - origin, normal)
                # But origin is just a point on plane.
                ...
                if score > best_score:
                    ...
                    # Refine origin to be mean of matched faces?
                    # No, usually we want bounding box limit or average level of the faces.
                    # Let's use average of the faces to be more precise than BB
                    # projected_dist = np.mean(np.dot(centers[matched_indices] - origin, normal))
                    # refined_origin = origin + normal * projected_dist

                    # Actually, for "Base Plane", we often want geometric extremum (BB)
                    # OR actual physical surface average.
                    # Let's use average of the faces to be safe (avoids noise outliers defining BB)
        ...
        try:
            ...
            for i in range(len(circular_loops)):
                ...
                if best_partner != -1:
                    ...
                    # We use RAY-CASTING to find TRUE depth (no more guessing!)
                    true_depth = self._measure_hole_depth_ray_cast(mesh, base_plane, tuple(c1), r1, tuple(n1))

                    hole = HoleInfo(
                        center=tuple(c1),
                        radius=float(r1),
                        depth=float(true_depth),  # TRUE DEPTH - NO MORE GUESSING!
                        axis=tuple(n1),
                        confidence=0.7,  # Lower confidence for single loop (ray-cast helps)
                        detection_method="visual_raycast"
                    )
                    holes.append(hole)
                    used_loops.add(i)
    ...
    def _measure_hole_depth_ray_cast(self, mesh, base_plane, center: Tuple[float, float, float],
                                    radius: float, normal: Tuple[float, float, float]) -> float:
        """
        Measure TRUE hole depth using ray-casting (no more guessing!).

        Args:
            mesh: PyVista mesh
            base_plane: PlaneInfo for reference
            center: 2D/3D center of hole
            radius: Hole radius
            normal: Hole axis direction
        ...
        try:
            ...
            # PyVista Ray-Casting
            # We create a line through the mesh and find intersections
            bounds = mesh.bounds
            line_length = max(
                bounds[1] - bounds[0],
                bounds[3] - bounds[2],
            ...
            # Using select_enclosed_points for pyvista >= 0.38
            try:
                inside_points = mesh.select_enclosed_points(line_points)
                inside_mask = inside_points["enclosed_points"]

                if np.sum(inside_mask) == 0:
                    # No intersection - use fallback
                    logger.warning(f"Ray-cast found no intersection for hole at {center}")
                    return radius * 2  # Last resort fallback

                # Find first and last inside point
                ...
                logger.debug(f"Ray-cast depth for hole at {center}: {depth:.2f}mm")
                return float(depth)

            except AttributeError:
                # Fallback for older pyvista versions
                # Use implicit point checking
                logger.warning("select_enclosed_points not available, using fallback")
                return radius * 2
    ...
    def _merge_holes(self, holes1: List[HoleInfo], holes2: List[HoleInfo]) -> List[HoleInfo]:
        """Merge two lists of holes, preferring second list (edge-based)."""
        merged = []
        if holes2:
    ...
    def _find_cylindrical_faces(self, mesh, centers: np.ndarray,
        ...
        # We'll stick to existing heuristic but allow more tolerance
        ...
        try:
            ...
            # Using mock implementation from Plan
    ...
    def _fit_hole_to_faces(self, mesh, face_indices: np.ndarray,
        ...
        try:
            ...
            if HAS_SKLEARN:
                ...
                # The null space of normals matrix is cylinder axis!
                # i.e. dot(n, axis) ~ 0
                U, S, Vt = np.linalg.svd(face_normals, full_matrices=False)
                axis = Vt[-1] # Direction with least variance in normals = perpendicular to all normals
            ...
            B = u**2 + v**2
            result, _, _ = np.linalg.lstsq(A, B, rcond=None)
            uc, vc, C = result
